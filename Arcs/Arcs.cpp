...
static const char USAGE_MESSAGE[] =
    PROGRAM " " PACKAGE_VERSION "\n"
            ...
            "       ARCS utilizes linked read alignments for scaffolding "
            ...
            "       ARKS scaffolds draft genomes using linked read kmers "
            "(https://doi.org/10.1186/s12859-018-2243-x).\n"
            "       Contig sequences are REQUIRED by the -f option.\n"
            "       linked read files are REQUIRED either as positional arguments or in a supplied "
            "file of linked read file paths. Pay attention to have only linked read files in the "
            "file of file names.\n"
            "\n"
            "       The barcode multiplicity file is optional and can be provided by -u option in "
            "either .tsv or .csv format.\n"
            "\n"
            "Common Options:\n"
            ...
            "   -r, --error_percent=N p-value for head/tail assignment and link orientation (lower "
            "is more stringent) [0.05]\n"
...
unsigned int s_numkmersmapped = 0, s_numkmercollisions = 0, s_numkmersremdup = 0, s_numbadkmers = 0,
             s_uniquedraftkmers = 0;

unsigned int s_totalnumckmers = 0, s_ckmersasdups = 0, s_numckmersfound = 0, s_numckmersrec = 0,
             s_numbadckmers = 0;
...
vector<string>
convertInputString(const string& inputString)
{
...
calcJacIndex(int smallCount, int overallCount)
{
...
static inline void
stripReadNum(std::string& readName)
{
 ...
 */
double
calcSequenceIdentity(const std::string& line, const std::string& cigar, const std::string& seq)
{
 ...
 * If they are all in same format pass-by-reference parameter allAlignment=1 tells they are all in
 * alignment format and allAlignment=0 if they are all in read file format.*/
bool
checkSameFormat(std::vector<std::string> fileNames, bool& allAlignment)
{
	...
	for (const auto& fileName : fileNames) {
		...
		if ((fileName.find(".sam") != npos) || (fileName.find(".bam") != npos))
			curType = 1;
		if ((fileName.find(".fastq") != npos) || (fileName.find(".fastq.gz") != npos) ||
		    (fileName.find(".fq") != npos) || (fileName.find(".fq.gz") != npos))
			curType = 2;
		if (!curType) {
 ...
 */
static inline bool
checkReadSequence(std::string seq)
{
...
void
createIndexMultMap(std::string multfile, std::unordered_map<std::string, int>& indexMultMap)
{
...
size_t
initContigArray(std::string contigfile)
{
	...
	if (params.verbose) {
		cerr << "Number of contigs:" << count << "\nSize of Contig Array:" << (count * 2) + 1
		     << endl;
	}
...
void
readBarcodes(vector<string> inputFiles, std::unordered_map<std::string, int>& indexMultMap)
{
...
void
getScaffSizes(std::string file, ARCS::ContigToLength& contigToLength)
{
    ...
    ARCS::ContigToLength& contigToLengthMap)
{
	...
	while (getline(bamName_stream, line)) {
		...
		if (line[0] == '@') {
			...
			if (startsWith(line, "@SQ\t")) {
				...
				if (addSAMSequenceLengths) {
					...
					if (it == contigToLengthMap.end()) {
						...
						std::cerr << "error: mismatched sequence lengths: sequence " << name << ": "
						          << it->second << " != " << size;
						exit(EXIT_FAILURE);
					}
				}
			...
			ss >> readName >> flag >> scafName >> pos >> mapq >> cigar >> rnext >> pnext >> tlen >>
			    seq >> qual >> std::ws;
			...
			/* Keep track of index multiplicity if alignment is not supplementary alignment and is
			 * not non-primary alignment*/
			...
			if (ct == 1) {
				if (readName.compare(prevRN) != 0) {
					...
					if (!readyToAddIndex.empty() && !readyToAddRefName.empty() &&
					    readyToAddRefName.compare("*") != 0 && readyToAddPos != -1) {
				...
				if (!seq.empty() && checkFlag(flag) && checkFlag(prevFlag) && mapq != 0 &&
				    prevMapq != 0 && si >= params.seq_id && prevSI >= params.seq_id) {
					if (prevRef.compare(scafName) == 0 && scafName.compare("*") != 0 &&
...
readFof(const std::string& fofName)
{
...
void
readBAMS(
    const std::vector<std::string> bamNames,
    ARCS::IndexMap& imap,
    std::unordered_map<std::string, int>& indexMultMap,
    ARCS::ContigToLength& contigToLengthMap)
{
...
/** Count barcodes. */
static size_t
countBarcodes(ARCS::IndexMap& imap, const std::unordered_map<std::string, int>& indexMultMap)
{
	...
	std::cout << "{ \"All_barcodes_unfiltered\":" << indexMultMap.size()
...
float
normalEstimation(int x, float p, int n)
{
	float mean = n * p;
 ...
 */
int
mapKmers(std::string seqToKmerize, int k, ARCS::ContigKMap& kmap, ReadsProcessor& proc, int conreci)
{
	...
	if (seqsize < k) {
		...
		while (i <= seqsize - k) {
			...
			if (temp != NULL) {
				...
				// ideally would want CAS operation but without special a datastructure this is
				// difficult
				bool exists;
				int alreadyconreci;
 ...
 */
int
bestContig(ARCS::ContigKMap& kmap, std::string readseq, int k, double j_index, ReadsProcessor& proc)
{
    ...
    ARCS::ContigToLength& contigToLength)
{
	...
	while ((l = kseq_read(seq)) >= 0) {
		...
		if (good) {
			// If the sequence is above minimum contig length, then will extract kmers from both
			// ends If not will ignore the contig
			...
			contigRecord[tempConreci1] =
			    headside; // in contigRecord head/tail of contig are kept as different contigs.
			std::string seqend;
			seqend = sequence.substr(0, cutOff);
			int num = mapKmers(seqend, params.k_value, kmap, proc, tempConreci1);

			totalKmers += num;

			contigRecord[tempConreci2] = tailside;
			seqend = sequence.substr(sequence_length - cutOff, sequence_length);
			num = mapKmers(seqend, params.k_value, kmap, proc, tempConreci2);
	...
	if (params.verbose) {
		printf(
		    "%s %u\n%s %u\n%s %u\n%s %u\n%s %u\n%s %u\n%s %u\n%s %u\n%s %u\n",
		    "Total number of contigs in draft genome: ",
		    totalNumContigs,
		    "Total valid contigs: ",
		    validContigs,
		    "Total skipped contigs: ",
		    skippedContigs,
		    "Total number of Kmers: ",
		    totalKmers,
		    "Number Null Kmers: ",
		    s_numbadkmers,
		    "Number Kmers Recorded: ",
		    s_numkmersmapped,
		    "Number Kmer Collisions: ",
		    s_numkmercollisions,
		    "Number Times Kmers Removed (since duplicate in different contig): ",
		    s_numkmersremdup,
		    "Number of unique kmers (only one contig): ",
		    s_uniquedraftkmers);
	}
}
...
#pragma omp critical(checkread1or2)
		{
			...
			if (read1_name == read2_name) {
				...
				std::cout << "File contains unpaired reads: " << read1_name << " " << read2_name
				          << std::endl;
				skipped_unpaired++;
			}
			count += 2;
...
#pragma omp atomic
			...
			if (paired && validbarcode && !barcode1.empty() && !barcode2.empty() &&
			    (barcode1 == barcode2)) {
				...
				if (goodmult && checkReadSequence(cread1) && checkReadSequence(cread2)) {
					corrConReci1 = bestContig(
					    kmap, cread1, params.k_value, params.j_index, *procs[omp_get_thread_num()]);
					corrConReci2 = bestContig(
					    kmap, cread2, params.k_value, params.j_index, *procs[omp_get_thread_num()]);
				} else {
#pragma omp atomic
					skipped_invalidreadpair++;
				}
				// we only store barcode info in index map if read pairs have same contig +
				// orientation and if the corrContigId is not NULL (because it is above accuracy
				// threshold)
...
#pragma omp atomic
	...
	if (params.verbose) {
		...
		if (emptybarcode > 0)
			printf(
			    "WARNING:: Your chromium read file has %d readpairs that have an empty barcode.",
			    emptybarcode);
		if (invalidbarcode > 0)
			printf(
			    "WARNING:: Your chromium read file has %d read pairs that have barcodes not in the "
			    "barcode multiplicity file.",
			    invalidbarcode);
	}
}
				...
				for (auto p = it->second.begin(); p != it->second.end(); ++p) {
					...
					if (scafA < scafB && scafAflag && scafBflag) {
						...
						std::tie(validA, scafAhead) = headOrTail(
						    it->second[ScaffoldEnd(scafA, true)],
						    it->second[ScaffoldEnd(scafA, false)]);
						std::tie(validB, scafBhead) = headOrTail(
						    it->second[ScaffoldEnd(scafB, true)],
						    it->second[ScaffoldEnd(scafB, false)]);
...
getMaxValueAndIndex(const ARCS::PairMap::value_type::second_type& array)
{
	unsigned max = 0;
	unsigned index = 0;
 ...
 */
bool
checkSignificance(int max, int second)
{
 ...
 */
void
createGraph(const ARCS::PairMap& pmap, ARCS::Graph& g)
{
 ...
 */
void
writeGraph(const std::string& graphFile_dot, ARCS::Graph& g)
{
 ...
 */
void
removeDegreeNodes(ARCS::Graph& g, int max_degree)
{
 ...
 */
void
writePostRemovalGraph(ARCS::Graph& g, const std::string graphFile)
{
	...
	if (params.max_degree != 0) {
		...
		std::cout << "      Max Degree (-d) set to: " << params.max_degree
		          << ". Will not delete any vertices from graph.\n";
	}
    ...
    DistGraph& gout)
{
	...
	for (const auto ein : boost::make_iterator_range(boost::edges(gin))) {
		...
		if (!inserted) {
			std::cerr << "error: Duplicate edge: \"" << get(vertex_name, gout, u) << "\" -> \""
 ...
 */
void
writeAbyssGraph(const std::string& path, const DistGraph& g)
{
	assert(!path.empty());
    ...
    const std::unordered_map<std::string, int>& indexMultMap)
{
	...
	sort(
	    sorted.begin(), sorted.end(), [](const Sorted::value_type& a, const Sorted::value_type& b) {
    ...
    size_t barcodeCount)
{
	...
	for (const auto& it : pmap) {
		...
		for (unsigned i = 0; i < counts.size(); ++i) {
			...
			bool vsense = i % 2;
			f << u << (usense ? '-' : '+') << '\t' << v << (vsense ? '-' : '+') << '\t'
			  << (counts[i] == max_counts ? "T" : "F") << '\t' << counts[i] << '\t'
			  << barcodes_per_scaffold_end[std::make_pair(u, usense)] << '\t'
			  << barcodes_per_scaffold_end[std::make_pair(v, !vsense)] << '\t' << barcodeCount
			  << '\n';
			f << v << (vsense ? '+' : '-') << '\t' << u << (usense ? '+' : '-') << '\t'
			  << (counts[i] == max_counts ? "T" : "F") << '\t' << counts[i] << '\t'
			  << barcodes_per_scaffold_end[std::make_pair(v, !vsense)] << '\t'
			  << barcodes_per_scaffold_end[std::make_pair(u, usense)] << '\t' << barcodeCount
			  << '\n';
		}
	}
	assert_good(f, tsvFile);
    ...
    ARCS::Graph& g)
{
	...
	time(&rawtime);
	std::cout << "\n\t=> Building Jaccard to distance map... " << ctime(&rawtime);
	JaccardToDist jaccardToDist;
	buildJaccardToDist(distSamples, jaccardToDist);

	time(&rawtime);
	std::cout << "\n\t=> Calculating barcode stats for scaffold pairs... " << ctime(&rawtime);
	...
	if (!params.dist_tsv.empty()) {
		time(&rawtime);
		std::cout << "\n\t=> Writing distance estimates to TSV... " << ctime(&rawtime);
...
/** Run ARCS. */
void
runArcs(const std::vector<std::string>& filenames)
{
	...
	std::cout << "\n -b " << maybeNA(params.base_name) << "\n -g "
	          << maybeNA(params.dist_graph_name)
	...
	if (!params.arks) {
		...
		time(&rawtime);
		std::cout << "\n=>Preprocessing: Gathering barcode multiplicity information..."
		          << ctime(&rawtime);
		if (!params.multfile.empty()) {
			createIndexMultMap(params.multfile, indexMultMap); // reading file
		} else {
			std::cout << "Multiplicity information is being formed from reads as no barcode "
			             "multiplicity file provided."
			          << std::endl;
...
main(int argc, char** argv)
{
	...
	for (int c; (c = getopt_long(argc, argv, shortopts, longopts, NULL)) != -1;) {
		...
		case OPT_BARCODE_COUNTS:
			arg >> params.barcode_counts_name;
			break;
		...
		case OPT_NO_DIST_EST:
			params.dist_est = false;
			break;
		case OPT_DIST_MEDIAN:
			params.dist_mode = ARCS::DIST_MEDIAN;
			break;
		case OPT_DIST_UPPER:
			params.dist_mode = ARCS::DIST_UPPER;
			break;
	...
	if ((params.arks && arcsOnly) || (!params.arks && arksOnly)) {
		cerr << PROGRAM ": error: You specified an option that does not match with method "
	...
	if (params.fofName.empty() && filenames.empty()) {
		cerr << PROGRAM ": error: specify input (SAM/BAM file(s) or chromium reads) or a list of "
		                "files with -a option\n";
		die = true;
	}
	...
	if (!stdIn && !checkSameFormat(filenames, alignmentFiles)) {
		std::cerr << "Input files must be all alignment or all read files." << params.file
		          << ". Exiting... \n";
		die = true;
	}
	/* Check if file type and method matches. */
	if (!stdIn && !(alignmentFiles ^ params.arks)) {
		std::cerr << "File type must be compatible with the method. (BAM/SAM for ARCS) or (Read "
		             "file for ARKS (--arks))"
		          << ". Exiting... \n";
		die = true;
	}
	...
	if (!g.good() && params.arks) {
		std::cerr << "Cannot find [-f] scaffold file which is required for --arks" << params.file
		          << ". Exiting... \n";
		die = true;
	}
	...
	if (params.arks) {
		...
		if (params.base_name.empty()) {
			...
			filename << params.file << ".scaff"
			         << "_arks"
			         << "_c" << params.min_reads << "_k" << params.k_value << "_j" << params.j_index
			         << "_l" << params.min_links << "_d" << params.max_degree << "_e"
