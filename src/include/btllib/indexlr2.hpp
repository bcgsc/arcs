...
public:
  ...
  Indexlr(std::string seqfile,
          ...
          bool verbose = false,
          size_t W = 0,
          std::vector<SpacedSeed> ss = std::vector<SpacedSeed>(),
...
private:
  ...
  std::vector<Minimizer> minimize_with_ss(const std::string& seq) const;
...
Indexlr::minimize(const std::string& seq) const
{
  ...
  std::vector<Minimizer> minimizers;
  std::vector<Minimizer> partial_minimizers;
  std::vector<Minimizer> paired_minimizers;
  partial_minimizers.reserve(seq.size() - k + 1 - w);
  paired_minimizers.reserve(seq.size() - W + 1);
  ...
  for (NtHash nh(seq, k, 2); nh.roll(); ++idx) {
    ...
    if (idx + 1 >= w) {
      ...
      if (!W) {
        ...
        partial_minimizers.push_back(*min_current);
      }
    }
  }
  uint64_t prev_min = std::numeric_limits<uint64_t>::max();
  for (size_t i = W/2; i < partial_minimizers.size(); ++i) {
    uint64_t curr_min = btllib::rolx(partial_minimizers[i].out_hash, k) ^ partial_minimizers[i - (W / 2)].out_hash;
    if (curr_min == prev_min) {
        continue;
    }
    prev_min = curr_min;
    Minimizer paired_min = HashedKmer (0, curr_min, 0, false, "");
    paired_minimizers.push_back(paired_min);
  }
  if (W) {
    std::cerr << "working345" << std::endl;
    return paired_minimizers;
  }
  return minimizers;
}

inline std::vector<Indexlr::Minimizer>
Indexlr::minimize_with_ss(const std::string& seq) const
{
  if ((k > seq.size()) || (w > seq.size() - k + 1)) {
    return {};
  }
  ...
  const Minimizer* min_current = nullptr;
  size_t idx = 0;
  for (SeedNtHash nh(seq, k, ss,2); nh.roll(); ++idx) {
    auto& hk = hashed_kmers_buffer[idx % hashed_kmers_buffer.size()];
...
Indexlr::MinimizeWorker::work()
{
  ...
  for (;;) {
    ...
    if (indexlr.k <= read.seq.size() &&
        ...
        if (indexlr.ss.size() == 0) {
          record.minimizers = indexlr.minimize(read.seq);
        } else {
            std::cerr << "working2" <<std::endl;
          record.minimizers = indexlr.minimize_with_ss(read.seq);
        }
    } else {
      record.minimizers = {};
    }
