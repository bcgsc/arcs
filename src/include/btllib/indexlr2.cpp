...
main(int argc, char* argv[])
{
	...
	bool w_set = false;
	bool k_set = false;
	int with_id = 0, with_bx = 0, with_readlen = 0, with_pos = 0, with_strand = 0, with_seq = 0;
	...
	static const struct option longopts[] = {
		...
		{ "len", no_argument, &with_readlen, 1 },   { "pos", no_argument, &with_pos, 1 },
	...
	for (auto& infile : infiles) {
		...
		if (with_repeat && with_solid) {
			...
			flags |= btllib::Indexlr::Flag::FILTER_OUT;
			indexlr = std::unique_ptr<btllib::Indexlr>(new btllib::Indexlr(
			    infile, k, w, flags, t, verbose, 0, repeat_bf->get_bloom_filter()));
		} else if (with_solid) {
			flags |= btllib::Indexlr::Flag::FILTER_IN;
			indexlr = std::unique_ptr<btllib::Indexlr>(
			    new btllib::Indexlr(infile, k, w, flags, t, verbose, 0, solid_bf->get_bloom_filter()));
		} else {
			indexlr = std::unique_ptr<btllib::Indexlr>(
